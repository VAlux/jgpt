...
public class Inference {
  ...
  public Inference(Model model, RandomGenerator randomGenerator, DataRepository dataRepository) {
    this.model = model;
  ...
  public ModelData train(Tokenizer tokenizer,
    ...
    var progress = new TrainingProgressReporter(iterationCount);
    ...
    for (int iteration = 0; iteration < iterationCount; iteration++) {
      ...
      progress.report(finishedIterationIndex, batchLoss.value(), movingAverageLoss, recentLosses);

      if (finishedIterationIndex % checkpointFrequency == 0) {
        progress.breakLine();
        System.out.printf("Checkpoint after %d iterations, saving...%n", finishedIterationIndex);
